"""HTTP server for the LLM agent with SSE streaming."""
...
async def startup() -> None:
    ...
    # Create MCP client
    _mcp_client = MCPClient(base_url="http://localhost:3003")
    ...
    try:
        ...
        logger.error("Make sure the MCP server is running: uv run python -m adk_analytics_server")
        raise
...
def run_server(host: str = "0.0.0.0", port: int = 3004, log_level: str = "info") -> None:
    ...
    logger.info(f"Starting agent server on http://{host}:{port}")
    logger.info("Make sure the MCP server is running on port 3003")
