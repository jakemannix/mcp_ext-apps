...
import type { Tile, Player, StartMazeResult, GenerateTileResult } from "./types";
...
function App() {
  ...
  const handleTileExit = useCallback(async (direction: "north" | "south" | "east" | "west") => {
  ...
  if (status === "connecting") {
    return <div style={styles.container}><div style={styles.status}>Connecting...</div></div>;
  }

  if (status === "error") {
    return <div style={styles.container}><div style={styles.error}>Error: {error?.message}</div></div>;
  }
