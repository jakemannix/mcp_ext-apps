...
start_agent_server() {
    ...
    cd "$AGENT_HOST_DIR"
    MCP_SERVER_URL=http://localhost:3002 uv run python -m agent --port 3004 &
    AGENT_SERVER_PID=$!
...
start_cloudflare_tunnel() {
    ...
    local tunnel_output=$(mktemp)

    # Start cloudflared in quick tunnel mode (no account needed)
    cloudflared tunnel --url http://localhost:3002 2>&1 | tee "$tunnel_output" &
    TUNNEL_PID=$!
