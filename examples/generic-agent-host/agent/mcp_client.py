...
class MCPClient:
    ...
    async def read_resource(self, uri: str) -> dict[str, Any]:
        ...
        Args:
            uri: Resource URI (e.g., "ui://maze/mcp-app.html")

        Returns:
            Resource content dict with 'contents' array
        """
    ...
    async def call_tool(self, name: str, arguments: dict[str, Any]) -> ToolResult:
        ...
        logger.info(f"Tool result: {text_content[:200]}..." if len(text_content) > 200 else f"Tool result: {text_content}")
        if structured_content:
            logger.info(f"Has structured content with keys: {list(structured_content.keys()) if isinstance(structured_content, dict) else 'non-dict'}")
        else:
            logger.warning(f"No structured content in result. Result keys: {list(result.keys()) if result else 'None'}")
