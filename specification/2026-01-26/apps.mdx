...
// Request
{
  ...
  params: {
    ...
    triggerAgent?: boolean  // If true, host MUST invoke agent immediately
  }
}
...
```
Host behavior:
* Host SHOULD add the message to the conversation context.
* Host MAY request user consent.
* If `triggerAgent` is `true`, Host MUST invoke the agent to process the message immediately and generate a response.
* If `triggerAgent` is `false` or omitted, Host MAY defer agent invocation until the next user-initiated turn.
...
This event serves a different use case from `notifications/message` (logging) and `ui/message` (which can trigger agent follow-ups via the `triggerAgent` flag).

Host behavior:
- SHOULD provide the context to the model in future turns
- MAY overwrite the previous model context with the new update
- MAY defer sending the context to the model until the next user message (including `ui/message` with `triggerAgent: true`)
