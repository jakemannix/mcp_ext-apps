...
```typescript
// Validate origin of incoming messages for security
const expectedOrigin = new URL(document.referrer).origin;
function isValidMessage(event: MessageEvent) {
  return event.source === window.parent && event.origin === expectedOrigin;
}

let nextId = 1;
function sendRequest(method: string, params: any) {
  const id = nextId++;
  window.parent.postMessage({ jsonrpc: "2.0", id, method, params }, expectedOrigin);
...
function sendNotification(method: string, params: any) {
  window.parent.postMessage({ jsonrpc: "2.0", method, params }, expectedOrigin);
}
