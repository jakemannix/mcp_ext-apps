...
interface McpUiAppCapabilities {
   ...
   * The intersection of host availableDisplayModes and app availableDisplayModes determines available options.
   *
...
The `availableDisplayModes` field allows apps to declare which display modes they support. This enables hosts to:

- Only switch to (and provide UI controls for) display mode options that the app can handle
- Calculate the intersection of host and app capabilities

**App Behavior:**

- Apps SHOULD support `inline` which is the primary display mode
