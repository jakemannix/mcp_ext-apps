...
5. The Sandbox MUST load the raw HTML of the Guest UI with CSP settings that:
   - Enforce the domains declared in the `ui.csp` resource metadata
   - Block nested iframes on domains NOT listed in `csp.frameDomains` (or on hosts that don't support the hostCapability.externalIframe cap)
